...
struct GpuModel::Impl {
    ...
    GpuProjection projection;GpuBsp bsp;GpuClip clip;GpuRaster raster;GpuColourWarp warp;
    ...
    GpuRasterOutput billboard(void* next_device,void* command,const assets::Shape& shape,
        ...
        bool surfaces,const GpuRasterOutput* background) {
        ...
        if(!texture || pose.simple_sprite_world_size<=0 || pose.z<128)
            return raster.enqueue_row_spans(device,command,nullptr,0,width*scale,height*scale,surfaces,nullptr,true,background);
        ...
        const bool dxil=(SDL_GetGPUShaderFormats(device)&SDL_GPU_SHADERFORMAT_DXIL)!=0;
            ...
            info.code=spirv?billboard_shader::spirv:dxil?billboard_shader::dxil:reinterpret_cast<const Uint8*>(billboard_shader::metal);
            info.code_size=spirv?sizeof(billboard_shader::spirv):dxil?sizeof(billboard_shader::dxil):std::strlen(billboard_shader::metal);
        ...
        struct Settings {
            std::array<float,4> camera_hi{},camera_lo{},view_hi{},view_lo{};
        ...
        for(unsigned i=0;i<4;++i) {
            config.camera_hi[i]=float(camera[i]);config.camera_lo[i]=float(camera[i]-config.camera_hi[i]);
            config.view_hi[i]=float(view[i]);config.view_lo[i]=float(view[i]-config.view_hi[i]);
        }
        ...
        return raster.enqueue_row_spans(device,command,billboard_spans,1,width*scale,rows,surfaces,buffers[9],true,background);
    }
    ...
    void* emit_surface(SDL_GPUCommandBuffer* command,void* camera,const SurfaceSettings& settings) {
        ...
        const bool dxil=(SDL_GetGPUShaderFormats(device)&SDL_GPU_SHADERFORMAT_DXIL)!=0;
            SDL_GPUComputePipelineCreateInfo info{};
            info.format=spirv?SDL_GPU_SHADERFORMAT_SPIRV:dxil?SDL_GPU_SHADERFORMAT_DXIL:SDL_GPU_SHADERFORMAT_MSL;
            info.code=spirv?surface_shader::spirv:dxil?surface_shader::dxil:reinterpret_cast<const Uint8*>(surface_shader::metal);
            info.code_size=spirv?sizeof(surface_shader::spirv):dxil?sizeof(surface_shader::dxil):std::strlen(surface_shader::metal);
...
GpuRasterOutput GpuModel::enqueue(void* device,void* command,const assets::Shape& shape,const RenderPose& unjittered_pose,
    const RenderSettings& settings,std::uint32_t width,std::uint32_t height,bool surface_metadata,const GpuRasterOutput* background,GpuModelDiagnostics* diagnostics,bool geometry_depth,GpuModelRaySource* ray_source,const RenderPose* previous_pose,std::array<float,2> raster_jitter) {
    auto pose=unjittered_pose;
    if(diagnostics) *diagnostics={};
    if(ray_source) *ray_source={};
    ...
    try {
        ...
        if(raster_jitter[0]!=0 || raster_jitter[1]!=0) {
            ...
            pose.vanish_x+=raster_jitter[0]/settings.render_scale;
            pose.vanish_y+=raster_jitter[1]/settings.render_scale;
        }
        if(pose.simple_scaled_sprite) {
            auto output=impl_->billboard(device,command,shape,pose,settings,width,height,surface_metadata,background);
        ...
        if(!vertex_count || faces.polygons.empty()) {
            auto output=impl_->raster.enqueue_row_spans(device,command,nullptr,0,width*settings.render_scale,height*settings.render_scale,surface_metadata,nullptr,true,background);
            if(!output.pixels) throw std::runtime_error(impl_->raster.status());
        ...
        std::vector<std::array<float,4>> near(colour_warp?slots:polygons,{float(pose.vanish_x),float(pose.vanish_y),float(settings.focal_length),accurate_fragment_clip?1.f:(vertices.continuous?2.f:0.f)});
        std::vector<std::array<std::int32_t,4>> normals;
        ...
        auto* spans=impl_->clip.enqueue_spans(command,materials,settings.render_scale>1,settings.render_scale,colour_warp?nullptr:&order,settings.wireframe_thickness,
            repeated_rows?b[9]:nullptr,repeated_rows?std::uint32_t(faces.texels.size()):0,repeated_rows?&masked_texels:nullptr);
        if(!spans) throw std::runtime_error(impl_->clip.status());
        const GpuGeometryDepthInput depth_input{impl_->geometry_planes,polygons,
            float(settings.focal_length*settings.render_scale),float(settings.focal_length*settings.render_scale),
            float((vertices.continuous?pose.vanish_x:vertices.native_pose.vanish[0])*settings.render_scale),
            float((vertices.continuous?pose.vanish_y:vertices.native_pose.vanish[1])*settings.render_scale)};
        auto output=impl_->raster.enqueue_row_spans(device,command,spans,slots,width*settings.render_scale,height*settings.render_scale,surface_metadata,repeated_rows?masked_texels:b[9],true,background,pose.wave_mode,pose.wave_offset,pose.animation_frame,
