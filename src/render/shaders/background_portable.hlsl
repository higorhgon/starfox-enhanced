...
cbuffer Settings : register(b0,space2) {
    ...
    uint tag,enabled,transparentBlack,padding;
};
...
void main(uint3 id:SV_DispatchThreadID) {
    ...
    if(priority && (((tile>>13)&1u)!=(priority==2u?1u:0u))) return;
    ...
    if(transparentBlack && !(memory[16384u+value]&32767u)) return;
